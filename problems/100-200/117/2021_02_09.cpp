 ...
 * method: BFS with the next link level by level
 * time: O(n), space: O(1)
 * Runtime: 12 ms, faster than 83.06% of C++ online submissions
 * Memory Usage: 17.2 MB, less than 97.39% of C++ online submissions
*/
...
public:
    Node* connect(Node* root) {
        ...
        while (levelHead != nullptr) {
            Node* head = levelHead, *nextLevelHead = nullptr;
            auto res = this->findNextHead(head, nextLevelHead);
            head = res.first; nextLevelHead = res.second;
            levelHead = nextLevelHead;
            while (head != nullptr) {
                res = this->findNextHead(head, nextLevelHead);
                nextLevelHead->next = res.second;
                head = res.first; nextLevelHead = res.second;
            }
        }
        return root;
...
private:
    pair<Node*, Node*> findNextHead(Node* head, Node* currNextLevelHead) {
        if (head == nullptr)
            return make_pair(nullptr, nullptr);
        if (currNextLevelHead == nullptr) {
            // find first head
            while (head != nullptr) {
                if (head->left != nullptr)
                    return make_pair(head, head->left);
                else if (head->right != nullptr)
                    return make_pair(head, head->right);
                head = head->next;
            }
            return make_pair(nullptr, nullptr);
        } else {
            // given the current next head
            if (head->left == currNextLevelHead && head->right != nullptr)
                return make_pair(head, head->right);
            else if (head->left == currNextLevelHead || head->right == currNextLevelHead) {
                head = head->next;
                if (head == nullptr)
                    return make_pair(nullptr, nullptr);
            }
